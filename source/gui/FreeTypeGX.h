...
typedef struct ftgxCharData_ {
    ...
    uint16_t
        glyphAdvanceX;   /**< Character glyph X coordinate advance in pixels. */
...
#define FTGX_COMPATIBILITY_GRRLIB                                              \
    FTGX_COMPATIBILITY_DEFAULT_TEVOP_GX_PASSCLR |                              \
        FTGX_COMPATIBILITY_DEFAULT_VTXDESC_GX_NONE
#define FTGX_COMPATIBILITY_LIBWIISPRITE                                        \
    FTGX_COMPATIBILITY_DEFAULT_TEVOP_GX_MODULATE |                             \
        FTGX_COMPATIBILITY_DEFAULT_VTXDESC_GX_DIRECT

const GXColor ftgxWhite =
    (GXColor){0xff, 0xff, 0xff, 0xff}; /**< Constant color value used only to
                                          sanitize Doxygen documentation. */
 ...
 * \version 0.2.4
 *
 * FreeTypeGX acts as a wrapper class for the libFreeType library. It supports
 * precaching of transformed glyph data into a specified texture format.
 * Rendering of the data to the EFB is accomplished through the application of
 * high performance GX texture functions resulting in high throughput of string
 * rendering.
 */
class FreeTypeGX {
  ...
  private:
    ...
    uint32_t compatibilityMode; /**< Compatibility mode for default tev
                                   operations and vertex descriptors. */
    std::map<wchar_t, ftgxCharData>
        fontData; /**< Map which holds the glyph data structures for the
    ...
    static int16_t getStyleOffsetHeight(ftgxDataOffset *offset,
                                        uint16_t format);
    ...
    void drawTextFeature(int16_t x, int16_t y, uint16_t width,
                         ftgxDataOffset *offsetData, uint16_t format,
                         GXColor color);
    void copyTextureToFramebuffer(GXTexObj *texObj, f32 texWidth, f32 texHeight,
                                  int16_t screenX, int16_t screenY,
                                  GXColor color);
    void copyFeatureToFramebuffer(f32 featureWidth, f32 featureHeight,
                                  int16_t screenX, int16_t screenY,
                                  GXColor color);
