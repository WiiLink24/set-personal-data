...
//! Sound conversion and playback. A wrapper for other sound libraries - ASND,
//! libmad, ltremor, etc
class GuiSound {
  public:
    //! Constructor
...
//! Menu input trigger management. Determine if action is neccessary based on
//! input data by comparing controller input data to a specific trigger element.
class GuiTrigger {
  public:
    //! Constructor
    GuiTrigger();
    ...
    u8 type;           //!< trigger type (TRIGGER_SIMPLE,	TRIGGER_HELD,
                       //!< TRIGGER_BUTTON_ONLY, TRIGGER_BUTTON_ONLY_IN_FOCUS)
};
...
class GuiElement {
  public:
    ...
    //! Considers horizontal alignment, x offset, width, and parent element's
    //! GetLeft() / GetWidth() values \return left coordinate
    int GetLeft();
    //! Gets the current topmost coordinate of the element
    //! Considers vertical alignment, y offset, height, and parent element's
    //! GetTop() / GetHeight() values \return top coordinate
    int GetTop();
    ...
    int GetMaxX();
    //! Gets the current width of the element. Does not currently consider the
    //! scale \return width
    int GetWidth();
    ...
    //! Callback function can be used to response to changes in the state of the
    //! element, and/or update the element's attributes
    ...
    //! Called when the language has changed, to obtain new text values for all
    //! text elements
  ...
  protected:
    GuiTrigger *trigger[3]; //!< GuiTriggers (input actions) that this element
                            //!< responds to
    UpdateCallback
        updateCB; //!< Callback function to call when this element is updated
    GuiElement *parentElement; //!< Parent element
    int focus;   //!< Element focus (-1 = focus disabled, 0 = not focused, 1 =
                 //!< focused)
    ...
    int xoffsetDyn; //!< Element X offset, dynamic (added to xoffset value for
                    //!< animation effects)
    int yoffsetDyn; //!< Element Y offset, dynamic (added to yoffset value for
                    //!< animation effects)
    int alpha;      //!< Element alpha value (0-255)
    int alphaDyn;   //!< Element alpha, dynamic (multiplied by alpha value for
                    //!< blending/fading effects)
    f32 xscale;     //!< Element X scale (1 = 100%)
    f32 yscale;     //!< Element Y scale (1 = 100%)
    f32 scaleDyn;   //!< Element scale, dynamic (multiplied by alpha value for
                    //!< blending/fading effects)
    int effects; //!< Currently enabled effect(s). 0 when no effects are enabled
    int effectAmount; //!< Effect amount. Used by different effects for
                      //!< different purposes
    int effectTarget; //!< Effect target amount. Used by different effects for
                      //!< different purposes
    int effectsOver;  //!< Effects to enable when wiimote cursor is over this
                      //!< element. Copied to effects variable on over event
    int effectAmountOver; //!< EffectAmount to set when wiimote cursor is over
                          //!< this element
    int effectTargetOver; //!< EffectTarget to set when wiimote cursor is over
                          //!< this element
    int alignmentHor;  //!< Horizontal element alignment, respective to parent
                       //!< element (LEFT, RIGHT, CENTRE)
    int alignmentVert; //!< Horizontal element alignment, respective to parent
                       //!< element (TOP, BOTTOM, MIDDLE)
    int state;         //!< Element state (DEFAULT, SELECTED, CLICKED, DISABLED)
    int stateChan;     //!< Which controller channel is responsible for the last
                       //!< change in state
    bool selectable; //!< Whether or not this element selectable (can change to
                     //!< SELECTED state)
    bool clickable; //!< Whether or not this element is clickable (can change to
                    //!< CLICKED state)
    bool holdable;  //!< Whether or not this element is holdable (can change to
                    //!< HELD state)
    bool visible;   //!< Visibility of the element. If false, Draw() is skipped
    bool rumble;    //!< Wiimote rumble (on/off) - set to on when this element
                    //!< requests a rumble event
};
...
class GuiWindow : public GuiElement {
  public:
    ...
    //!\param s State
    void SetState(int s);
    //! Gets the index of the GuiElement inside the window that is currently
    //! selected \return index of selected GuiElement
    int GetSelected();
  ...
  protected:
    std::vector<GuiElement *>
        _elements; //!< Contains all elements within the GuiWindow
};

//! Converts image data into GX-useable RGBA8. Currently designed for use only
//! with PNG files
class GuiImageData {
  public:
    //! Constructor
...
//! Display, manage, and manipulate buttons in the GUI. Buttons can have images,
//! icons, text, and sound set (all of which are optional)
