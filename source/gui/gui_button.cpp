...
void GuiButton::SetSoundOver(GuiSound *snd) { soundOver = snd; }
void GuiButton::SetSoundHold(GuiSound *snd) { soundHold = snd; }
void GuiButton::SetSoundClick(GuiSound *snd) { soundClick = snd; }
...
void GuiButton::Update(GuiTrigger *t) {
    ...
    if (t->wpad->ir.valid && t->chan >= 0) {
        if (this->IsInside(t->wpad->ir.x, t->wpad->ir.y)) {
            ...
            if (state == STATE_SELECTED &&
                (stateChan == t->chan || stateChan == -1))
                this->ResetState();

            if (effectTarget == effectTargetOver &&
                effectAmount == effectAmountOver) {
    ...
    if (this->IsClickable()) {
        ...
        for (int i = 0; i < 3; i++) {
            if (trigger[i] &&
                (trigger[i]->chan == -1 || trigger[i]->chan == t->chan)) {
                ...
                if ((t->wpad->btns_d > 0 &&
                     (wm_btns == wm_btns_trig ||
                      (cc_btns == cc_btns_trig &&
                       t->wpad->exp.type == EXP_CLASSIC))) ||
        ...
        for (int i = 0; i < 3; i++) {
            if (trigger[i] &&
                ...
                if (!held && state == STATE_HELD && stateChan == t->chan) {
                    this->ResetState();
                } else if (held && state == STATE_CLICKED &&
                           stateChan == t->chan) {
