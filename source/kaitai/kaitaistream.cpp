...
#elif defined(HW_RVL) // !__APPLE__ or !_MSC_VER
#include <machine/endian.h>

// We appear to only be given inline variants from devkitPPC.
// This will suffice.
#define bswap_16(x) __bswap16(x)
#define bswap_32(x) __bswap32(x)
#define bswap_64(x) __bswap64(x)

// We additionally want to prefer encoding conversion via iconv.
#define KS_STR_ENCODING_ICONV 1

#else // !__APPLE__ or !_MSC_VER or !HW_RVL
#include <byteswap.h>
#include <endian.h>
#endif
...
void kaitai::kstream::exceptions_enable() const {
    m_io->exceptions(std::istream::eofbit | std::istream::failbit |
                     std::istream::badbit);
}
...
void kaitai::kstream::seek(uint64_t pos) { m_io->seekg(pos); }

uint64_t kaitai::kstream::pos() { return m_io->tellg(); }
    ...
    if (bits_needed > 0) {
        ...
        if (bytes_needed > 8)
            throw std::runtime_error(
                "read_bits_int: more than 8 bytes requested");
        char buf[8];
...
uint64_t kaitai::kstream::read_bits_int(int n) { return read_bits_int_be(n); }
...
uint64_t kaitai::kstream::read_bits_int_le(int n) {
    ...
    if (bits_needed > 0) {
        ...
        if (bytes_needed > 8)
            throw std::runtime_error(
                "read_bits_int_le: more than 8 bytes requested");
        char buf[8];
        m_io->read(buf, bytes_needed);
        for (int i = 0; i < bytes_needed; i++) {
            uint8_t b = buf[i];
...
std::string kaitai::kstream::read_bytes(std::streamsize len) {
    ...
    // NOTE: streamsize type is signed, negative values are only *supposed* to
    // not be used. http://en.cppreference.com/w/cpp/io/streamsize
    if (len < 0) {
...
std::string kaitai::kstream::read_bytes_term(char term, bool include,
...
std::string kaitai::kstream::ensure_fixed_contents(std::string expected) {
    ...
    if (actual != expected) {
        // NOTE: I think printing it outright is not best idea, it could contain
        // non-ascii charactes like backspace and beeps and whatnot. It would be
        // better to print hexlified version, and also to redirect it to stderr.
        throw std::runtime_error(
            "ensure_fixed_contents: actual data does not match expected data");
    }
...
std::string kaitai::kstream::bytes_terminate(std::string src, char term,
                                             bool include) {
...
std::string kaitai::kstream::process_xor_many(std::string data,
                                              std::string key) {
    size_t len = data.length();
...
std::string kaitai::kstream::process_zlib(std::string data) {
    ...
    do {
        ...
        if (outstring.size() < strm.total_out)
            outstring.append(reinterpret_cast<char *>(outbuffer),
                             strm.total_out - outstring.size());
    } while (ret == Z_OK);
...
std::string kaitai::kstream::to_string(int val) {
    ...
    if (got_len > (int)sizeof(buf))
        throw std::invalid_argument(
            "to_string: integer is longer than string buffer");
    ...
    while (true) {
        ...
        if (res == (size_t)-1) {
            if (iconv_error == E2BIG) {
                ...
                // dst.resize might have allocated destination buffer in another
                // area of memory, thus our previous pointer "dst" will be
