...
class kstream {
  public:
     ...
     * \param pos new position (offset in bytes from the beginning of the
     * stream)
     */
     ...
     * \return pointer position, number of bytes from the beginning of the
     * stream
     */
    uint64_t pos();
    ...
    static std::string bytes_terminate(std::string src, char term,
                                       bool include);
    ...
    /**
     * Performs a XOR processing with given data, XORing every byte of input
    ...
    /**
     * Performs a XOR processing with given data, XORing every byte of input
     * with a key array, repeating key array many times, if necessary (i.e. if
     * data array is longer than key array).
     * @param data data to process
     ...
     * amount of bits, using groups of 1 bytes each time. Right circular
     * rotation should be performed using this procedure with corrected amount.
...
#ifdef KS_ZLIB
    /**
     * Performs an unpacking ("inflation") of zlib-compressed data with usual
     * zlib headers.
